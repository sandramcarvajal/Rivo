...
export const RouteCard: React.FC<RouteCardProps> = ({ route, onClick, className }) => {
  return (
    <motion.div
      ...
      className={cn(
        'bg-white border border-slate-100 rounded-[24px] p-6 shadow-soft hover:border-blue-200 transition-all cursor-pointer group',
        className
      )}
    >
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center space-x-3">
          <div className="relative">
            <img
              src={`https://ui-avatars.com/api/?name=${route.driverName}&background=E0F2FE&color=0369A1`}
          ...
          <div>
            <h4 className="font-bold text-slate-800 leading-tight">{route.driverName}</h4>
      ...
      <div className="space-y-4 relative mb-6 pl-2">
        ...
        <div className="flex items-center space-x-4">
          ...
          <p className="text-xs font-semibold text-slate-500 truncate">{route.origin}</p>
        </div>
        <div className="flex items-center space-x-4">
          <div className="w-2.5 h-2.5 rounded-full border-2 border-primary bg-white z-10" />
          <p className="text-xs font-bold text-slate-800 truncate">{route.destination}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-5 border-t border-slate-50">
        ...
        <div className="flex items-center bg-slate-50 px-3 py-1.5 rounded-xl">
